...
#	 Obtain fetal fraction from a iSNP GVCF file
#	 Change the GVF and output path for each run
# Note: only consider maternal and parental genotype
# ****************************************************************
...
# Set output file
output='/dssg/home/acct-medkwf/medkwf4/results/NIPT-M/merge/filter_v2.vcf'
rm -fr ${output}
touch ${output}
...
do
  ...
  if [ "$parental" = "0/0" ] && [ "$maternal" = "1/1" ]; then
    echo $line
    ...
    echo $line >> ${output}
  elif [ "$parental" = "1/1" ] && [ "$maternal" = "0/0" ]; then
    echo $line
    WTreads=$(echo $line | tr -d '\n' | awk '{n=split($12,a,/:/);split(a[2],b,/,/);print b[1]}')
    SNPreads=$(echo $line | tr -d '\n' | awk '{n=split($12,a,/:/);split(a[2],b,/,/);print b[2]}')
    depth=`calc $SNPreads+$WTreads`
